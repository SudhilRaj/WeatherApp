...
const App = () => {
  ...
  const getWeather = useDebouncedCallback(() => {
    if(!!city.trim()){
      fetch(`${constants.API_URL}?q=${city.trim()}&units=metric&appid=${constants.WEATHER_MAP_API_KEY}`)
  ...
  return (
    <div className={`${darkMode ? 'dark' : ''}`}>
      <HelmetHead />
      <div className='flex flex-col items-center justify-between p-6 sm:p-8 md:p-12 !pt-14 h-screen bg-cover bg-center 
      ...
      >
        ...
        <div className='flex flex-col items-center w-full'>
          ...
          <div className='w-full sm:w-3/4 lg:w-1/2 relative'>
            <input
              ...
              onKeyUp={getWeather}
              autoComplete='off'
              className='w-full m-0 p-3 text-neutral-900 bg-slate-200 border-none rounded sm:rounded-md
              text-md sm:text-xl focus:outline-none ring-offset-2 ring ring-gray-200'
            />
            {!!city && (
              <span className='absolute top-3 right-2 cursor-pointer' onClick={clearSearch}>
        ...
        {weatherData?.main && (
          ...
          <div className='w-3/4 lg:w-1/2 my-5'>
            <div className='h-auto min-h-full flex flex-col sm:flex-row flex-wrap items-center justify-around
